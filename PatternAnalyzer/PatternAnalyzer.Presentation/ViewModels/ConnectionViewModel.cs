...
public partial class ConnectionViewModel : ObservableObject
{
    ...
    public void LoadConnectionInfo(DbConnectionInfo info)
    {
        SelectedDatabaseType = info.Type;
        Host = info.Host;
        Port = info.Port;
        DatabaseName = info.DatabaseName;
        Username = info.Username;
        // Password is explicitly NOT loaded from preset as it is not saved
        StatusMessage = "Connection info loaded from preset. Please enter password.";
        IsConnected = false;
        Tables.Clear();
        Columns.Clear();
    }
